...
private:
	// Function to check if a point is valid (inside the image and not yet visited)
	bool isValid(const cv::Mat &image, int x, int y, const cv::Mat &visited)
	{
		return (x >= 0 && y >= 0 && x < image.cols && y < image.rows && !visited.at<uchar>(y, x));
	}

	void regionGrowing(const cv::Mat &depthImage, const std::vector<cv::Point> &contour, const cv::Point &startPoint, cv::Mat &visited, double &averageDepth, int &validPixelCount);
