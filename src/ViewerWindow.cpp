...
void ViewerWindow::Render() {
    ...
    while (!Terminated && !glfwWindowShouldClose(window)) {
        ...
        if (!depth.empty() && (tracker.IsTrackingTools() || calibrationInitiated))
        {
            ...
            ImGui::Image(reinterpret_cast<void *>(static_cast<intptr_t>(texture)), ImVec2(256, 256));
            ImGui::End();
            ...
            ImGui::Image(reinterpret_cast<void *>(static_cast<intptr_t>(dtexture)), ImVec2(256, 256));
            ImGui::End();
        }
