...
void IRToolTracker::AddFrame(void* pAbImage, void* pDepth, uint32_t depthWidth, uint32_t depthHeight, cv::Mat _pose, double _timestamp) {
	...
	cv::Mat cvAbImage_origin(cv::Size(depthWidth, depthHeight), CV_16UC1, (void*)pAbImage);
...
// Function to perform region growing from a point
void IRToolTracker::regionGrowing(const cv::Mat &depthImage, const std::vector<cv::Point> &contour, const cv::Point &startPoint, cv::Mat &visited, double &averageDepth, int &validPixelCount)
{
	...
	while (!stack.empty())
	{
		...
		if (!isValid(depthImage, x, y, visited))
			continue;
		...
		if (cv::pointPolygonTest(contour, cv::Point2f(static_cast<float>(x), static_cast<float>(y)), false) >= 0)
		{
			// std::cout << "Depth at (" << x << ", " << y << "): " << depthImage.at<uint16_t>(y, x) << std::endl;
	...
	// Compute average depth if there are valid pixels
	if (validPixelCount > 0)
	{
		averageDepth = depthSum / validPixelCount;
	}
	else
	{
		averageDepth = 0;
...
bool IRToolTracker::ProcessFrame(AHATFrame* rawFrame, ProcessedAHATFrame &result) {
	...
	cv::Mat depthImage(rawFrame->depthHeight, rawFrame->depthWidth, CV_16U, rawFrame->pDepth);

	double perimeter, circ;
	...
	for (int i = 1; i < areaCount; ++i)
	{
		...
		if (area <= maxSize && area >= minSize)
		{
			...
			// Create a visited map to keep track of the visited pixels
			cv::Mat visited = cv::Mat::zeros(depthImage.size(), CV_8U);

			for (const auto &contour : contours)
			{
				perimeter = cv::arcLength(contour, true); // true for closed contour
				circ = (4 * PI * area) / (perimeter * perimeter);
				if (circ < 0.7f)
					continue; // filter by circularity

				// Start region growing from the centroid
				cv::Point centroid(static_cast<int>(_u), static_cast<int>(_v));
				double averageDepth = 0;
				int validPixelCount = 0;

				regionGrowing(depthImage, contour, centroid, visited, averageDepth, validPixelCount);
				if (validPixelCount == 0)
				{
					continue;
				}
				...
				float uv[2] = {_u, _v};
				float xy[2] = {0, 0};
				float depth = (static_cast<float>(averageDepth));
